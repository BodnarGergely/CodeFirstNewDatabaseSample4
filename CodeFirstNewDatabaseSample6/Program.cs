...
namespace CodeFirstNewDatabaseSample6
{
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new BloggingContext())
            {
               ...
               /* var ticket6 = new Ticket { PassangerId = 3, Title = "Havi bérlet", StartOfValidity = new DateTime(2015, 4, 17, 12, 2, 15), StopOfValidity = (new DateTime(2015, 4, 17, 12, 2, 15)).AddMonths(1) };
                ...
                Console.WriteLine("All Passangers and his tickets:");
                foreach (var item in query)
                {
                    /*---------------------------------------------*/
                    var ticketName =  db.Tickets
                        .Where (p=>p.PassangerId ==item.PassangerId).FirstOrDefault();
                    /*---------------------------------------------*/

                    Console.WriteLine("Name: {0}, and his ticket: {1}, Start Time: {2}, Stop Time: {3}", item.Name, ticketName.Title, ticketName.StartOfValidity, ticketName.StopOfValidity);
                }
